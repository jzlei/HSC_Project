function hopf_br=Bif_tauPM(dim)
k = dim;
min_bd = 3;
max_bd = 10.5;
addpath(genpath('./dde_biftool'));
min_kappaP = 1.5;2.5/1000;
max_kappaP = 15;
addpath(genpath('./dde_biftool'));
...
ss2 = ss;
ss2.parameter(1) = ss.parameter(1) + 0.01;
[ss2, ss_succ] = p_correc(funcs,ss2,[],[],method.point);
ss2.stability = p_stabil(funcs,ss2,method.stability);
ss2.x;
ss_br = df_brnch(funcs,[1],'stst',1);
ss_br.point = ss;
ss_br.point(2) = ss2;
ss_br.parameter.min_bound=[1 0.3];
ss_br.parameter.max_bound=[1 15];
ss_br.parameter.max_step=[1 0.05];
[ss_br, succ,fail,rjct] = br_contn(funcs,ss_br,1500);
hopf_br=br_rvers(ss_br);
[ss_br, succ,fail,rjct] = br_contn(funcs,ss_br,1500);
ss_br=br_stabl(funcs,ss_br,0,0);
[xm,ym]=df_measr(1,ss_br);
figure;
br_plot(ss_br,xm,ym,'b'); % plot stability along branch:
ym.subfield='l0';
...
[hopf,success1]=p_correc(funcs,hf,[1],[],method.point);


hopf_br=df_brnch(funcs,[1 4],'hopf');
hopf_br.parameter.min_bound(1:2,:)=[[1 min_kappaP]' [4 min_bd]']'; 
hopf_br.parameter.max_bound(1:2,:)=[[1 max_kappaP]' [4 max_bd]']'; 
hopf_br.parameter.max_step(1:2,:)=[[1 min(0.01, min_bd/5)]' [4 0.01]']';
hopf_br.point=hopf;

hf.parameter(4)=hf.parameter(4)+0.01;
[hopf,success2]=p_correc(funcs,hf,1,[],method.point);
